...
#include<math.h>
void gen_element(int max_height,int height,int begin,int end,CP::map_bst<int,int> &bst){
  int x = (begin+end)/2;
  bst[x] = 100;
  if(height!=max_height){
    gen_element(max_height,height+1,begin,x-1,bst);
    gen_element(max_height,height+1,x+1,end,bst);
  }
}
